...
'''
    ...
    Examples:
      python local_annotator.py --output path/to/output --somatic_variants path/to/somatic_mutations.tsv --ascatestimates path/to/ascat.tsv
      ...
      python local_annotator.py --output path/to/output --somatic_variants path/to/somatic_mutations.tsv --cn_annotations path/to/cnas --pid patient_id --ascatestimates path/to/ascat.tsv
      python local_annotator.py --output path/to/output --somatic_variants path/to/somatic_mutations.tsv --post_annotate --ascatestimates path/to/ascat.tsv
'''
def main(**kwargs):
    """
...
def process_somatic_variants(kwargs):
    ...
    somatic_mutationdf = pd.DataFrame(joined)
    somatic_mutationdf.to_csv(output, sep='\t')
    print(somatic_mutationdf)
...
def post_annotate_somatic_variants(kwargs):
    ...
    ascats = pd.read_csv(kwargs["ascatestimates"], sep="\t", encoding='utf-8')
    somatic_mutations = pd.read_csv(kwargs["somatic_variants"], sep="\t", encoding='utf-8')
    ...
    somatic_mutation_grps = somatic_mutations.groupby('sample_id')

    for gname, cna_grp in cna_grps:
        process_sample(gname, cna_grp, somatic_mutation_grps, output, cores, ascats)

def process_sample(gname, cna_grp, somatic_mutation_grps, output, cores, ascats):
    output_file = f"{output}/{gname}_somatic_mutations.csv"
    ...
    somatic_mutation_grp = somatic_mutation_grps.get_group(gname)
    somatic_mutations_filtered = df_apply(somatic_mutation_grp, annotator.post_filter_and_classify_somatic_mutations_by_sample, None, None, True, True, cores=cores)
    somatic_mutationdf = pd.DataFrame(somatic_mutations_filtered)
    somatic_mutationdf.to_csv(output_file, sep='\t', header=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tumortype", type=str, default=CGI_DEFAULT_CANCER_TYPE, help=f"Tumor type identifier (default: {CGI_DEFAULT_CANCER_TYPE})")
    parser.add_argument("--refgen", type=str, default=CGI_DEFAULT_REFERENCE, help=f"Reference genome version (default: {CGI_DEFAULT_REFERENCE})")
